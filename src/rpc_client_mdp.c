...
int rpc_client_call_mdp (sid_t server_sid, char *rpc_name, int paramc, char **params, uint32_t requirements) {
	_rpc_eval_event(1, 3, "calling MDP", alloca_tohex_sid_t(server_sid), rpc_name);
	...
	while (received == 0 || received == 1) {
		...
		if (received == 0) {
			// Send the payload.
			_rpc_eval_event(1, 3, "sending call MDP", alloca_tohex_sid_t(server_sid), rpc_name);
        ...
        if (fds->revents & POLLIN) {
			...
			_rpc_eval_event(1, 3, "received data MDP", alloca_tohex_sid_t(mdp_recv_header.remote.sid), rpc_name);
	        ...
	        if (pkt_type == RPC_PKT_CALL_ACK) {
				_rpc_eval_event(1, 3, "received ACK MDP", alloca_tohex_sid_t(mdp_recv_header.remote.sid), rpc_name);
				...
				_rpc_eval_event(1, 3, "received result MDP", alloca_tohex_sid_t(mdp_recv_header.remote.sid), rpc_name);
